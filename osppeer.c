...
static void remove_and_advance(task_t* t, peer_t* current, peer_t **peer_list, int **count) {
    ...
    if (current->next)
    {
        ...
        peer_t *temp = current->prev;

        task_pop_peer(t, temp, peer_list, count);
    } else {
...
{
    //error("we get here? %d\n", current->port);
    if (current->addr.s_addr == listen_addr.s_addr && current->port == listen_port)
    {
        error("peer pop\n");
...
int main(int argc, char *argv[])
{
    ...
    if ((pwent = getpwuid(getuid())))
    {
        ...
        sprintf((char *) myalias, "%s%d", pwent->pw_name, (int) time(NULL));
    } else {
